...
class EchoPrototypeMod {
    ...
    companion object {
        const val MOD_ID = "player_traces_echo_prototype"
    }
}

internal object EchoPrototypeKeys {
    val quarkWave = KeyMapping("key.player_traces_echo_prototype.quark_wave", GLFW.GLFW_KEY_F4, CATEGORY)
    val syntheticBend = KeyMapping("key.player_traces_echo_prototype.synthetic", GLFW.GLFW_KEY_F6, CATEGORY)
    val duration = KeyMapping("key.player_traces_echo_prototype.duration", GLFW.GLFW_KEY_F7, CATEGORY)
    val record = KeyMapping("key.player_traces_echo_prototype.record", GLFW.GLFW_KEY_F8, CATEGORY)
    ...
    private const val CATEGORY = "key.categories.player_traces_echo_prototype"
}
