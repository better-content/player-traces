...
import com.bettercontent.traces.logic.GuidanceProgressTracker
import com.bettercontent.traces.client.TracesClientRenderer
import com.bettercontent.traces.client.GuidancePathModel
import com.bettercontent.traces.client.SurfaceAnchorResolver
import com.bettercontent.traces.client.TraceVisualModel
import com.bettercontent.traces.client.TraceSightOverlayModel
import com.bettercontent.traces.client.TraceSightOverlayTransition
import com.bettercontent.traces.network.TraceQueryResponsePacket
import com.bettercontent.traces.network.TraceAnnotationsSeenPacket
...
class TraceCoreTest {
    ...
    fun movementCaptureUsesPreciseThreeQuarterBlockSpacing() {
        assertEquals(0.74, com.bettercontent.traces.logic.CaptureService.horizontalDistance(0.74, 0.0, 0.0, 0.0), 0.0001)
        assertEquals(0.75, com.bettercontent.traces.logic.CaptureService.horizontalDistance(0.45, 0.60, 0.0, 0.0), 0.0001)
        val sampled = com.bettercontent.traces.logic.CaptureService.sampleSegment(
            Vec3(0.0, 64.0, 0.0), Vec3(2.25, 64.3, 2.25), 0.0,
        )
        assertEquals(4, sampled.points.size)
        assertTrue(sampled.points.zipWithNext().all { (a, b) ->
            kotlin.math.abs(com.bettercontent.traces.logic.CaptureService.horizontalDistance(a.x, a.z, b.x, b.z) - 0.75) < 0.0001
        })
    ...
    fun serializerRejectsNegativeSeenRevision(@TempDir dir: Path) {
        ...
        val state = TraceShardState()
        state.seenStates += com.bettercontent.traces.storage.SeenStateRecord(
            UUID.randomUUID(),
            UUID.randomUUID(),
            -1,
        )
    ...
    fun guidanceProgressAwardsForwardDistanceOnceAndRejectsTeleportProgress() {
        ...
        fun route(start: Int) = com.bettercontent.traces.dto.GuidanceRouteDto(
        ...
        val longRoute = com.bettercontent.traces.dto.GuidanceRouteDto(
    ...
    fun traceQueryResponsePacketCarriesAnnotationSeenFlag() {
        val packet = TraceQueryResponsePacket(
            ...
            guidanceRoutes = listOf(
                com.bettercontent.traces.dto.GuidanceRouteDto("ann", 2, listOf(GuidancePointDto(0.25, 64.0, 0.75), GuidancePointDto(5.5, 64.0, 6.5))),
            ),
        )
        val buffer = FriendlyByteBuf(Unpooled.buffer())
    ...
    fun annotationMutationPacketsAreBoundedAndRevisionChecked() {
        ...
        assertThrows(IllegalArgumentException::class.java) {
            com.bettercontent.traces.network.AnnotationCreatePacket(BlockPos.ZERO.asLong(), oversized)
        ...
        assertThrows(IllegalArgumentException::class.java) {
            com.bettercontent.traces.network.AnnotationUpdatePacket.decode(negative)
        }
    }
